#include <iostream>

// Include the glm library
#include <glm/glm.hpp>
#include <glm/gtx/io.hpp>

// Prototype Exercise6 (must Prototype all functions aside from Main!)
int Exercise6();

int main() {
    // 4.2. Vectors
    printf("Vectors and Matrices\n");
    printf("--------------------------------------------------\n");

    // Defining Vectors
    glm::vec3 a, b;
    a = glm::vec3(3.0f, 0.0f, 4.0f);
    b[0] = 1.0f, b[1] = 2.0f, b[2] = 3.0f;

    printf("Defining Vectors\n");
    std::cout << "a = " << a << std::endl;
    std::cout << "b = " << a << std::endl;

    // 4.2.1. Vector Magnitude
    printf("\nVector length:\n");
    printf("length(a) = %0.3f\n", glm::length(a));
    printf("length(b) = %0.3f\n", glm::length(b));

    // 4.2.2. Unit Vectors
    glm::vec3 aHat, bHat;
    aHat = glm::normalize(a);
    bHat = b / glm::length(b);

    printf("\nNormalizing vectors:\n");
    std::cout << "aHat = " << aHat << std::endl;
    std::cout << "bHat = " << bHat << std::endl;
    printf("length(aHat) = %0.3f\n", glm::length(aHat));
    printf("length(bHat) = %0.3f\n", glm::length(bHat));

    // 4.3. Arithmetic Operations on Vectors
    // 4.3.1. Vector Addition and Subtraction
    printf("\nArithmetic Operations on Vectors:\n");
    std::cout << "a + b = " << a + b << std::endl;
    std::cout << "a - b = " << a - b << std::endl;
    // 4.3.2. Multiplication and Division by a Scalar
    std::cout << "2a    = " << 2.0f * a << std::endl;
    std::cout << "b / 3 = " << b / 3.0f << std::endl;
    // 4.3.3. Element-wise Multiplication
    std::cout << "a * b = " << a * b << std::endl;

    // 4.3.4. The Dot Product
    printf("\n Dot and Cross Products:\n");
    printf("a . b = %0.3f\n", glm::dot(a, b));
    // 4.3.5. The Cross Product
    std::cout << "a x b = " << glm::cross(a, b) << std::endl;
    printf("a . (a x b) = %0.3f\n", glm::dot(a, glm::cross(a, b)));
    printf("b . (a x b) = %0.3f\n", glm::dot(b, glm::cross(a, b)));

    // 4.4. Matrices
    glm::mat2 A, B;
    A[0][0] = 1.0f, A[0][1] = 2.0f;
    A[1][0] = 3.0f, A[1][1] = 4.0f;
    B = glm::mat2(5.0f, 6.0f, 7.0f, 8.0f);
    
    // 4.4.2. Matrix Transpose
    printf("\nDefining Matrices:\n");
    std::cout << "A = " << glm::transpose(A) << "\n" << std::endl;
    std::cout << "B = " << glm::transpose(B) << std::endl;

    // 4.5. Arithmetic Operations on Matrices
    printf("\nArithmetic operations on matrices:\n");
    std::cout << "A + B = " << glm::transpose(A + B) << "\n" << std::endl;
    std::cout << "A - B = " << glm::transpose(A - B) << "\n" << std::endl;
    std::cout << "2A    = " << glm::transpose(2.0f * A) << "\n" << std::endl;
    std::cout << "A / 3 = " << glm::transpose(A / 3.0f) << "\n" << std::endl;

    // 4.5.1. Matrix Multiplication
    // WARNING: MATRIX MULTIPLICATION SHOULD ALWAYS "OPPOSITE ACTUALLY" WHEN CALCULATING!
    // IT *CANNOT* BE DONE EITHER WAY AROUND!!
    std::cout << "A * B = " << glm::transpose(B * A) << "\n" << std::endl;
    std::cout << "B * A = " << glm::transpose(A * B) << "\n" << std::endl;

    // 4.5.2. The Identity Matrix
    printf("\nThe Identity Matrix\n");
    glm::mat4 I;
    std::cout << "I = " << glm::transpose(I) << "\n" << std::endl;

    // 4.5.3. Inverse Matrices
    printf("\nInverse matrices:\n");
    glm::mat2 invA = glm::inverse(A);
    glm::mat2 invB = glm::inverse(B);
    std::cout << "invA = " << glm::transpose(invA) << "\n" << std::endl;
    std::cout << "invB = " << glm::transpose(invB) << "\n" << std::endl;
    std::cout << "invA * A = " << glm::transpose(A * invA) << "\n" << std::endl;
    std::cout << "invB * B = " << glm::transpose(B * invB) << "\n" << std::endl;

    // 4.6. Excercises
    Exercise6();

    return 0;
}

int Exercise6() {
    // 4.6.2. Points
    glm::vec3 x, y, z; // A, B, C, without repeating previous variables
    x = glm::vec3(5.0f,  1.0f,  3.0f);
    y = glm::vec3(10.0f, 7.0f,  4.0f);
    z = glm::vec3(0.0f,  5.0f, -3.0f);

    // 4.6.2a. Vector p that points from x to y
    glm::vec3 p = y - x;
    // 4.6.2b. Vector q that points from y to z
    glm::vec3 q = z - y;
    // 4.6.2c. Vector r that points from z to x
    glm::vec3 r = x - z;
    // 4.6.2d. Length of Vector p
    float pLength = glm::length(p);
    // 4.6.2e. Unit Vector pointing at Vector q
    glm::vec3 qHat = glm::normalize(q);
    // 4.6.2f. Dot Product of p and q
    float pqDot = glm::dot(p, q);
    // 4.6.2g. Cross Product of q and r
    float qrCross = glm::dot(q, r);

    printf("Exercises\n");
    printf("--------------------------------------------------\n");
    std::cout << "p = " << p << std::endl;
    std::cout << "q = " << q << std::endl;
    std::cout << "r = " << r << std::endl;
    printf("\nlength(p) = %0.3f\n", pLength);
    printf("length(qHat) = %0.3f\n", glm::length(qHat));
    printf("p . q = %0.3f\n", pqDot);
    printf("q x r = %0.3f\n", qrCross);

    return 0;
}